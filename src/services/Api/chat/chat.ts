...
import { ChatResponse, ConversationResponse, startConversationRequest, UploadMediaResponse } from "@/types/chat"
...
export const getConversationMedia = async (conversationId: string,page: number = 1, limit: number = 10) => {
    try {
        const response = await axiosService.get<ConversationMessagesResponse>(`api/conversation/getMedia/${conversationId}?page=${page}&limit=${limit}`)
        return response
    } catch (error) {
